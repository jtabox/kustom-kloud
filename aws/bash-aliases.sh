...
cecho() {
    ...
    if [ "$#" -eq 1 ]; then
        if [[ $1 == "--help" ]]; then
            ...
            echo -e "${CLR_UFG_ORANGE}Usage:${CLR_CLEAR} ${CLR_FG_BLUE}cecho${CLR_CLEAR} ${CLR_FG_CYAN}[color]${CLR_CLEAR} ${CLR_FG_LIGHTBLUE}<message>${CLR_CLEAR}\n"
            echo -e "${CLR_UFG_ORANGE}Available colors:${CLR_CLEAR}"
            echo -e "${CLR_FG_ORANGE}[color] can be one of the following colors, +b for background color, l+ for light tint.\nNo [color] functions as a simple 'echo -e' :${CLR_CLEAR}"
            echo -e "${CLR_UFG_ORANGE}[color]:${CLR_CLEAR} ${CLR_FG_BLUE}blue${CLR_CLEAR}, ${CLR_FG_GRAY}gray/grey${CLR_CLEAR}, ${CLR_FG_GREEN}green${CLR_CLEAR}, ${CLR_FG_MAGENTA}magenta${CLR_CLEAR}, ${CLR_FG_RED}red${CLR_CLEAR}, ${CLR_FG_YELLOW}yellow${CLR_CLEAR}"
            echo -e "${CLR_UFG_ORANGE}    +b :${CLR_CLEAR} ${CLR_BG_CYAN}cyan${CLR_CLEAR}, ${CLR_BG_GRAY}gray/grey${CLR_CLEAR}, ${CLR_BG_GREEN}green${CLR_CLEAR}, ${CLR_BG_MAGENTA}magenta${CLR_CLEAR}, ${CLR_BG_RED}red${CLR_CLEAR}, ${CLR_BG_YELLOW}yellow${CLR_CLEAR}"
            echo -e "${CLR_UFG_ORANGE}    l+ :${CLR_CLEAR} ${CLR_FG_LIGHTBLUE}lblue${CLR_CLEAR}, ${CLR_FG_LIGHTGREEN}lgreen${CLR_CLEAR}, ${CLR_FG_ORANGE}orange${CLR_CLEAR}"
            return
        fi
        local message=${@:1}
